...
class KTags
{
    ...
    function show($params, $node)
    {
        ...
        if ($as_json && is_array($value)) {
            $value = $FUNCS->json_encode($value);
        }
        return $value;
    }
    ...
    function k_break($params, $node)
    {
        global $CTX;
        if (count($node->children)) {
            die("ERROR: Tag \"break\" is a self closing tag");
        }
    ...
    function k_continue($params, $node)
    {
        global $CTX;
        if (count($node->children)) {
            die("ERROR: Tag \"continue\" is a self closing tag");
        }
    ...
    function editable($params, $node, $called_from_repeatable = 0)
    {
        ...
        if ($called_from_repeatable) {
            $attr['k_desc'] = $attr['desc'];
            unset($attr['desc']);
            $attr['k_type'] = $attr['type'];
            unset($attr['type']);
            $attr['k_order'] = $attr['order'];
            unset($attr['order']);
            $attr['k_separator'] = $attr['separator'];
            unset($attr['separator']);
            $attr['k_group'] = ($attr['type'] == 'group') ? '' : $attr['group'];
            unset($attr['group']);
            $attr['_html'] = $tag;
        ...
        if (!$name) {
            die("ERROR: Tag \"" . $node->name . "\" needs a 'name' attribute");
        }
        $found = 0;
...
FORM;
                ...
                if ($FUNCS->is_non_zero_natural($special)) { // 1, 2, etc.
                    ...
                    if ($special == 'current') {
                        ...
                        if ($arr[0] == 'current' && $FUNCS->is_non_zero_natural($arr[1])) {
                            ...
                            if ($special == -1) { // _ROOT_
                                if ($root) {
                                    return; /* cannot add _ROOT_ to returned array */
                                } else {
                                    $childof = '';
                                }
                            } else {
    ...
    function _skip_menu(&$item)
    {
        ...
        if (!is_null($item->access_callback)) {
            ...
            if (!$callback) {
                ob_end_clean();
                die("ERROR: skip_menu callback function for menu-item '" . $item->name . "' is not callable.");
            }
    ...
    function field($params, $node)
    { // for use within cms:config_list_view/config_form_view
        ...
        if ($parent_tag == 'config_list_view') {
            ...
            if (trim($valid_params['group']) == '') {
                $valid_params['group'] = null;
            }
    ...
    function persist($params, $node)
    { // for use within config_form_view only
        global $CTX, $FUNCS, $AUTH;
        if ($AUTH->user->access_level < K_ACCESS_LEVEL_SUPER_ADMIN) {
            return;
        }
    ...
    function input($params, $node)
    {
        ...
        $type = strtolower(trim($type));
        if (!$type) {
            die("ERROR: Tag \"" . $node->name . "\" needs a 'type' attribute");
        }
        ...
        if (!$FUNCS->is_core_formfield_type($type)) {
            ...
            if (array_key_exists($type, $FUNCS->udform_fields)) {
                ...
                if (!is_array($attr_udf)) $attr_udf = array();
                if (array_key_exists('type', $attr_udf)) {
                    unset($attr_udf['type']);
    ...
    function google_map($params, $node)
    {
        ...
        if (!K_GOOGLE_KEY) {
            die("ERROR: Tag \"" . $node->name . "\" requires a valid Google API key in config.php");
        }
        ...
        if ($PAGE->google_map == 0) {
            $html .= "<script src=\"//maps.googleapis.com/maps/api/js?v=3&amp;key=$key\"></script>";
        }
...
MAP;
    ...
    function gpc($params, $node)
    {
        ...
        if (!in_array($method, array('get', 'post', 'cookie'))) {
            $method = '';
        }
    ...
    function masquerade($params, $node)
    {
        ...
        if (!preg_match('/^\s*(?:http:\/\/|https:\/\/)/i', $url)) {
            die("ERROR: Tag \"" . $node->name . "\" - url must begin with http:// or https://");
        }
    ...
    function translate_icon($params, $node)
    { // translate icon name to current theme's icon-set
        global $FUNCS;
        extract($FUNCS->get_named_vars(
            array(
                'name' => ''
    ...
    function show_icon($params, $node)
    { // show rendered icon
        global $FUNCS;
        extract($FUNCS->get_named_vars(
            array(
    ...
    function validate($params, $node)
    {
        ...
        if (!strlen($validator)) {
            die("ERROR: Tag \"" . $node->name . "\" requires a 'validator' parameter");
        }
        $separator = trim($separator);
