...
class KWebpage
{
    ...
    function _fill_page_info($skip_custom_fields)
    {
        ...
        (is_null($this->id) && is_null($this->page_name))) {
    ...
    function create_draft()
    {
        ...
        if ($rs != 1) {
            $DB->rollback();
            return $FUNCS->raise_error("Failed to insert record in K_TBL_PAGES for draft");
        }
        $page_id = $DB->last_insert_id;
    ...
    function _recreate_parent()
    {
        ...
        if ($rs != 1) {
            $DB->rollback();
            return $FUNCS->raise_error("Failed to insert record in K_TBL_PAGES");
        }
