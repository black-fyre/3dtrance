...
class KFuncs
{
    ...
    function add_event_listener($event_name, $listener, $priority = 0)
    {
        if ($listener = $this->is_callable($listener, true)) {
            ...
            ob_end_clean();
            die("ERROR: function add_event_listener(): \"" . $event_name . "\" - listener function is not callable");
        }
    }
    ...
    { // Used for saving searchable version of inputted text
    ...
    function register_admin_menuitem($params)
    {
        ...
        if (!is_array($params)) {
            die("ERROR: function register_admin_menuitem() requires an array as parameter");
        }
    ...
    function _register_menuitem($params, $array_name, $func_name)
    {
        ...
        if (!$menuitem['name']) {
            die("ERROR: function $func_name() requires a 'name' attribute");
        }
        if (array_key_exists($menuitem['name'], $this->$array_name)) {
            ob_end_clean();
            die("ERROR: function $func_name(): '" . $menuitem['name'] . "' already registered");
        }
    ...
    function register_admin_sub_menuitem($params)
    {
        ...
        if (!is_array($params)) {
            die("ERROR: function register_admin_sub_menuitem() requires an array as parameter");
        }
    ...
    function add_admin_action($params, $func_name = null)
    {
        ...
        if (!is_array($params)) {
            die("ERROR: function add_admin_action() requires an array as parameter");
        }
    ...
    function add_form_field($params)
    {
        ...
        if (!is_array($params)) {
            die("ERROR: function add_form_field() requires an array as parameter");
        }
        ...
        unset($params['label']);
        $params['menu_text'] = $params['desc'];
        unset($params['desc']);
        $params['weight'] = $params['order'];
        unset($params['order']);
        $params['parent'] = $params['group'];
        unset($params['group']);
        $params['show_in_menu'] = !($params['skip']);
        unset($params['skip']);
    ...
    function add_list_field($params)
    {
        ...
        if (!is_array($params)) {
            die("ERROR: function add_list_field() requires an array as parameter");
        }
        ...
        if (!$item['name']) {
            die("ERROR: function add_list_field() requires a 'name' attribute");
        }
        if (array_key_exists($item['name'], $this->admin_list_fields)) {
            ob_end_clean();
            die("ERROR: function add_list_field(): '" . $item['name'] . "' already registered");
        }
    ...
    function set_admin_list_default_limit($limit)
    {
        ...
        if (!$this->is_non_zero_natural($limit)) {
            $limit = '15';
        }
    ...
    function render($name)
    {
        ...
        if (!$entry['template_path']) { // renderable is function
            if ($entry['include_file']) {
                require_once($entry['include_file']);
            }
    ...
    function get_current_desktop_time()
    { //+5.5 India
    ...
    function register_validator($name, $callable)
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function register_validator(): Please provide a name to register the validator");
        }
        if (array_key_exists($name, $this->validators)) {
            ob_end_clean();
            die("ERROR: function register_validator(): '$name' already registered");
        }

        $callable = $this->is_callable($callable, 1 /*syntax only*/);
        if (!$callable) {
            ob_end_clean();
            die("ERROR: function register_validator(): Please provide a valid callable for the validator '$name'");
        }
    ...
    { // now used only by the install routine
...
OUT;
    ...
    function insert_comment($params, $node)
    {
        ...
        if (!$PAGE->tpl_is_clonable || ($PAGE->tpl_is_clonable && !$PAGE->is_master)) {
            ...
            if ($PAGE->tpl_is_commentable && $PAGE->comments_open) {
                ...
                if ($rs != 1) {
                    $DB->rollback();
                    return $this->raise_error("Failed to insert record in K_TBL_COMMENTS");
                }
    ...
    function register_udf($fieldtype, $handler_class, $repeatable = 0, $searchable = 1)
    { //'searchable' or not applies only to text types (numerics are always non_searchable)
    ...
    function register_spl_template($name, $handler, $include_file = '')
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function register_spl_template(): Please provide a name");
        }
        if (array_key_exists($name, $this->spl_templates)) {
            ob_end_clean();
            die("ERROR: function register_spl_template(): \"" . $name . "\" already registered");
        }
        if (!($handler = $this->is_callable($handler, true /*check syntax_only*/))) {
            ob_end_clean();
            die("ERROR: function register_spl_template(): \"" . $name . "\" - handler function is not callable");
        }
    ...
    function handle_spl_template($name, $args)
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function handle_spl_template(): Please provide a name");
        }
        if (!array_key_exists($name, $this->spl_templates)) {
            ob_end_clean();
            die("ERROR: function handle_spl_template(): \"" . $name . "\" not registered");
        }
    ...
    function register_render($name, $params)
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function register_render(): Please provide a name");
        }
        if (array_key_exists($name, $this->renderables)) {
            ob_end_clean();
            die("ERROR: function register_render(): \"" . $name . "\" already registered");
        }
    ...
    private function _register_render($name, $params, $func_name)
    {
        if (!is_array($params)) {
            ob_end_clean();
            die("ERROR: function " . $func_name . "(): \"" . $name . "\" requires an array as second parameter");
        }
        ...
        if ($entry['template_path']) { // renderable is a template
            if ($entry['template_ctx_setter'] && !($entry['template_ctx_setter'] = $this->is_callable($entry['template_ctx_setter'], true /* check syntax_only */))) {
                ob_end_clean();
                die("ERROR: function " . $func_name . "(): \"" . $name . "\" - 'template_ctx_setter' is not callable");
            }
        } else { // renderable is a function
            if (!($entry['renderable'] = $this->is_callable($entry['renderable'], true /* check syntax_only */))) {
                ob_end_clean();
                die("ERROR: function " . $func_name . "(): \"" . $name . "\" - renderable function is not callable");
            }
        }
    ...
    function override_render($name, $params)
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function override_render(): Please provide a name");
        }
    ...
    function register_route($masterpage, $params)
    {
        ...
        if (!$masterpage) {
            ob_end_clean();
            die("ERROR: function register_route(): Please provide a template-name to register the route for");
        }
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function register_route(): Please provide a name for the route");
        }
        if (array_key_exists($name, $this->routes[$masterpage])) {
            ob_end_clean();
            die("ERROR: function register_route(): '$name' already registered for module '$masterpage'");
        }
        $path = trim($path);
        ...
        if ($masterpage != '') {
            ...
            if (is_array($routes) && count($routes)) {
                ...
                if ($found) {
                    ...
                    if ($route->include_file) {
                        require_once($route->include_file);
                    }
    ...
    function generate_route($masterpage, $name, $values = array())
    {
        ...
        if ($masterpage == '' || $name == '') {
            return '#';
        } // No masterpage or route name - no link
    ...
    function register_filter($name, $callable)
    {
        ...
        if (!$name) {
            ob_end_clean();
            die("ERROR: function register_filter(): Please provide a name to register the filter");
        }
        if (array_key_exists($name, $this->filters)) {
            ob_end_clean();
            die("ERROR: function register_filter(): '$name' already registered");
        }

        $callable = $this->is_callable($callable, 1 /*syntax only*/);
        if (!$callable) {
            ob_end_clean();
            die("ERROR: function register_filter(): Please provide a valid callable for the filter '$name'");
        }
