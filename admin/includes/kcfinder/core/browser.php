...
class browser extends uploader
{
    ...
    public function __construct()
    {
        ...
        if ((
                !is_dir($thumbsDir) &&
                !$this->_mkdir($thumbsDir, $this->config['dirPerms'])
            ) ||
            ...
            (
                ...
                !$this->_mkdir("$thumbsDir/{$this->type}", $this->config['dirPerms'])
            )
        )
    ...
    protected function act_newDir()
    {
        ...
        if (!$this->_mkdir("$dir/$newDir", $this->config['dirPerms']))
    ...
    protected function act_rename()
    {
        ...
        // Couch thumbs
        $thumbs = dir::get_couch_thumbs($this->post['file'], $dir);
        foreach ($thumbs as $thumb) {
            $thumb_name = basename($thumb);
            $parent_orig_name = file::getName($this->post['file']);
            if (strpos($thumb_name, $parent_orig_name) === 0) {
                $new_thumb_name = file::getName($this->post['newName']) . substr($thumb_name, strlen($parent_orig_name));
                @rename($thumb, "$dir/$new_thumb_name");
            }
        }
        ...
        foreach ($this->post['files'] as $file) {
            ...
            elseif (!@copy($path, "$dir/$base"))
                ...
                if (is_file($fromThumb) && is_readable($fromThumb)) {
                    ...
                    if (!is_dir($toThumb))
                        $this->_mkdir($toThumb, $this->config['dirPerms'], true);
                    $toThumb .= "/$base";
                    @copy($fromThumb, $toThumb);
                    if (function_exists("chmod"))
                        @chmod($toThumb, $this->config['filePerms']);
                }

                // Couch thumbs
                $thumbs = dir::get_couch_thumbs($path);
                foreach ($thumbs as $thumb) {
                    $toThumb = $dir . '/' . basename($thumb);
                    @copy($thumb, $toThumb);
                }
            }
        }
