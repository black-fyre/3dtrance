    ...
    static function _render_richtext($f, $input_name, $input_id, $extra, $dynamic_insertion)
    {
        ...
        if ($repeatable) {
            ...
            if ($dynamic_insertion) { // being added on-the-fly.. output template markup that will be duplicated for each dynamically added new row
                ...
                ob_start();
                ?>
                <img src="<?php echo K_SYSTEM_THEME_URL; ?>assets/blank.gif" alt=""
                     id="<?php echo $input_id ?>_dummyimg" onload='
    ...
    function _render($input_name, $input_id, $extra = '', $dynamic_insertion = 0)
    {
        ...
        if (!$this->page->tpl_nested_pages) {
            ...
            <div class="ctrls-radio">
                <label for="f_publish_status_1" onClick="$('#publish-date').show()"><input type="radio"
                                                                                           <?php if ($publish_date != '0000-00-00 00:00:00'){ ?>checked="checked"<?php } ?>
                                                                                           value="1"
                                                                                           id="f_publish_status_1"
                                                                                           name="f_publish_status"/><span
                            class="ctrl-option"></span><?php echo $FUNCS->t('published'); ?></label>
                <label for="f_publish_status_0" onClick="$('#publish-date').hide()"><input type="radio"
                                                                                           <?php if ($publish_date == '0000-00-00 00:00:00'){ ?>checked="checked"<?php } ?>
                                                                                           value="0"
                                                                                           id="f_publish_status_0"
                                                                                           name="f_publish_status"/><span
                            class="ctrl-option"></span><?php echo $FUNCS->t('unpublished'); ?></label>
            </div>
            <div id="publish-date" style="display:<?php if ($publish_date == '0000-00-00 00:00:00') {
                echo 'none';
            } else {
                echo 'block';
            } ?>;">
            ...
            ?>
            <div class="ctrls-radio">
                <label for="f_publish_status_1"><input type="radio"
                                                       <?php if ($publish_date != '0000-00-00 00:00:00'){ ?>checked="checked"<?php } ?>
                                                       value="1" id="f_publish_status_1" name="f_publish_status"/><span
                            class="ctrl-option"></span><?php echo $FUNCS->t('active'); ?></label>
                <label for="f_publish_status_0"><input type="radio"
                                                       <?php if ($publish_date == '0000-00-00 00:00:00'){ ?>checked="checked"<?php } ?>
                                                       value="0" id="f_publish_status_0" name="f_publish_status"/><span
                            class="ctrl-option"></span><?php echo $FUNCS->t('inactive'); ?></label>
            </div>
...
class KUserDefinedField extends KField
{
    ...
    // Load from database
    function store_data_from_saved($data)
    { // just duplicating the default logic of KField.
        $this->data = $data;
    }
